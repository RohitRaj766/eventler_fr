...
export const ACTION_SPECS = Object.fromEntries(ACTIONS.map((spec) => [spec.action, spec]));
...
export const ROLE_BLUEPRINTS = [
    {
        ...
        description: 'Runs the organization: members, roles, events, venues and the audit log.',
    ...
    },
    {
        key: 'event-admin',
        name: 'Event Admin',
        description: 'Full control over event structure, schedule, tasks and venues — but no access to members, roles or billing.',
        category: 'COORDINATOR',
        builtIn: false,
        scopeCaveat: 'Intended to be scoped to one event. The API has no event-level membership, so this role currently applies to every event in the organization.',
        actions: [
            ACTION.programRead,
    ...
    },
    {
        ...
        description: 'Builds and runs the schedule: structure, dependencies, timings and tasks.',
        category: 'COORDINATOR',
        builtIn: true,
        scopeCaveat: 'Applies organization-wide until the API supports event-level membership.',
        actions: [
            ACTION.programRead,
            ACTION.nodeCreate, ACTION.nodeRead, ACTION.nodeUpdate,
            ACTION.timelineRead, ACTION.timelineUpdate,
    ...
    },
    {
        key: 'operator',
        name: 'Operator',
        description: 'Runs the event on the day: reads the run sheet and records what actually happened. Cannot restructure the event.',
        category: 'COORDINATOR',
        builtIn: false,
        scopeCaveat: 'Applies organization-wide until the API supports event-level membership.',
        actions: [
    ...
    },
    {
        key: 'task-manager',
        name: 'Task Manager',
        description: 'Owns the task board: creates, assigns and tracks work. Cannot change the schedule.',
        category: 'COORDINATOR',
        builtIn: false,
        actions: [
    ...
    },
    {
        key: 'volunteer',
        name: 'Volunteer',
        description: 'Sees the schedule and their own tasks, and updates the tasks assigned to them.',
        category: 'VOLUNTEER',
        builtIn: true,
        actions: [
            ACTION.programRead, ACTION.nodeRead, ACTION.timelineRead,
 ...
 */
export function resolveReach(permissions) {
    const held = new Set(permissions);
    const isSuperAdmin = held.has(SUPER_ADMIN_WILDCARD);
    const reach = new Map();
    const volunteerShaped = !isSuperAdmin && held.has(ACTION.taskUpdate) && !held.has(ACTION.taskCreate);
    for (const spec of ACTIONS) {
        ...
        reach.set(spec.action, spec.action === ACTION.taskUpdate && volunteerShaped ? 'assigned' : 'organization');
    }
    return reach;
}
 ...
 */
export function coversSubject(reach, userId, subject) {
    if (reach === 'none')
        return false;
    if (reach === 'organization')
        return true;
    if (!subject)
        return true; // capability question, not an instance question
 ...
 */
export function canDelegate(granterPermissions, requestedActions) {
    const held = new Set(granterPermissions);
    if (held.has(SUPER_ADMIN_WILDCARD))
...
export function delegatableActions(granterPermissions) {
    const held = new Set(granterPermissions);
    if (held.has(SUPER_ADMIN_WILDCARD))
        return ACTIONS;
...
export function unenforcedActions() {
